...
func queryOrders(db *gorm.DB) error {
	query := db.Raw(`
		SELECT orders.id, customers.name AS customer_name, customers.address AS customer_address, orders.payment_method, orderitems.total, orders.status, orders.created_at
		FROM orders
		JOIN (
